...
def mask_outliers_gaussian_process(x_values, y_values, alpha_values):
    ...
    masked_values = []
    kernel = GPR_kernel_smoother()
    
    c = 0
    factor = 2
    while c == 0:
        gaussian_process_model = GPR_model(x_values[mask], 
                                           y_values[mask], 
                                           kernel, 
                                           alpha=alpha_values[mask])

        mean_prediction, std_prediction = GPR_predict(gaussian_process_model,
                                                      x_values[mask])

        diff = mean_prediction - y_values[mask]
        
        ### use standard deviation of prediction (aka envelope)
